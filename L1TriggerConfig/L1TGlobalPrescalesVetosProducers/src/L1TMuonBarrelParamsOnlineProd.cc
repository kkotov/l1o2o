...
boost::shared_ptr<L1TMuonBarrelParams> L1TMuonBarrelParamsOnlineProd::newObject(const std::string& objectKey, const L1TMuonBarrelParamsO2ORcd& record) {
    ...
    if( objectKey.empty() ){
        edm::LogError( "L1-O2O: L1TMuonBarrelParamsOnlineProd" ) << "Key is empty";
...
///        return boost::shared_ptr< L1TMuonBarrelParams > ( new L1TMuonBarrelParams( *(baseSettings.product()) ) );
    ...
    if( queryResult.queryFailed() || queryResult.numberRows() != 1 ){
        edm::LogError( "L1-O2O" ) << "Cannot get BMTF_KEYS.{ALGO,HW} for ID = " << tscKey ;
...
///        return boost::shared_ptr< L1TMuonBarrelParams > ( new L1TMuonBarrelParams( *(baseSettings.product()) ) );
    ...
    // select MP7, DAQTTC from CMS_TRG_L1_CONF.BMTF_RS_KEYS where ID = rsKey ;
    queryResult =
            m_omdsReader.basicQuery( queryStrings,
                                     stage2Schema,
                                     "BMTF_RS_KEYS",
    ...
    if( queryResult.queryFailed() || queryResult.numberRows() != 1 ){
        edm::LogError( "L1-O2O" ) << "Cannot get BMTF_RS_KEYS.{MP7,DAQTTC} for ID = " << rsKey ;
        throw std::runtime_error("Broken key");
///        return boost::shared_ptr< L1TMuonBarrelParams > ( new L1TMuonBarrelParams( *(baseSettings.product()) ) );
    }
