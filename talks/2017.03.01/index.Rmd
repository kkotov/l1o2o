...
payload was prepared for EMTF but never used as the pT LUT neven changed in 2016 (expected to change in 2017)
...
Splitting a monolitic job in a [sequence per-payload jobs](https://github.com/cms-sw/cmssw/blob/CMSSW_9_0_X/L1TriggerConfig/Utilities/test/runOneByOne.sh) doubles running time (13s $\rightarrow$ 25s)
* for per-payload monitoring I'd prefer mimicing multiple jobs by splitting logs of one job
...
* with the new L1CE service and new FM ploicy this problem is not expected to reoccur

<span style="font-weight:bold;color:brown">CALO:</span>
eta-compress LUT is not in online DB $\rightarrow$ prototype has to be [kept-in-synch](https://github.com/cms-sw/cmssw/blob/CMSSW_9_0_X/L1TriggerConfig/Utilities/test/uploadCaloParams.py) by experts 
...
<span style="font-weight:bold;color:brown">EMTF:</span> add pT LUT EventSetup configurability to the new emulator
* new FW version at p5, although O2Oed, is not very useful if unknown to the emulator
* contrary, pT LUT "Forests" result from an offline study and transfered to p5 later

--- .class #id


## L1T payloads for Monte Carlo

https://cms-conddb.cern.ch/cmsDbBrowser/list/Prod/tags/L1TGlobalPrescalesVetos_passThrough
